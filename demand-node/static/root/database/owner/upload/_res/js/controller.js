var app = angular.module('ownerUploadCtrlM', ['toastr']);
app.controller('ownerUploadCtrl', function($scope,projectSer,$state,toastr,$stateParams,$http){
    $scope.files = [];
    $scope.affirmFile = [];
    var oldFiles = [];
    //删除文件
    ...
    $scope.updataSel = function () {
        ...
        if (_files.length) {
            ...
            fd.append('tonken', $stateParams.tonken);
            fd.append('type', $stateParams.type);
            $http({
                method: 'POST',
                url: '/owner/upload',
                headers: {
                ...
                if (response.data.code == 200) {
                    ...
                    $scope.$emit('changeMenu','listMenu');
                    $state.go('root.database.owner.list[12]',{id:$stateParams.id,tonken:$stateParams.tonken});
                    $scope.files = [];//预览的数组
